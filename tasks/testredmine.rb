...
#*********************************TEST*********************************************
  ...
  #   index_user = names.index('second_user two')
  #   @index2 = index_user.to_i + 1
  #
  #   expected_text = 'second_user two'
  #   actual_text = @driver.find_element(:css, ".member:nth-child("+(@index2.to_s)+") .name .user").text
  #   assert_equal(expected_text, actual_text)
  # end
  ...
  def test_edit_user_roles
    ...
    @wait.until { @driver.find_element(:css, ".member:nth-child("+(@index.to_s)+") span[id*='member']").displayed? }
    expected_text = 'Reporter'
    actual_text = @driver.find_element(:css, ".member:nth-child("+(@index.to_s)+") span[id*='member']").text
    assert_equal(expected_text, actual_text)
  end
